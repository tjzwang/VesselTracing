...
shuffle_input = '/Users/zacharyhoglund/Downloads/2131/tau_intensity_binned_data_MEGASHEET 10.csv';
...
Shuffledata = readmatrix(shuffle_input);
...
for i = 1:length(Shuffledata)

    Shuffledata2(i,:) = [Shuffledata(i,1:6) mean(Shuffledata(i,17:(17+surf_d))) Shuffledata(i,17:(17+ves_dist)) ];
...
% Calculate standard deviation and median
shufflestd = std(Shuffledata2(:,7));
shufflemed = median(Shuffledata2(:,7));


shuffled = Shuffledata(:,7);
shuffles = shufflestd;
shufflem = shufflemed;
...
edges = 0:(100/perc_binW):100;
   ...
   Shufflepercentiles = invprctile(Shuffledata2(:,7),Shuffledata2(:,7));
   
   Shufflebin = discretize(Shufflepercentiles,edges);

   Shuffledata3 = [Shufflebin Shuffledata2];
...
for i = 1:max(Shufflebin)

   
    idx = find(Shuffledata3(:,1) == i);

    Shuffleplotdatatemp = Shuffledata3(idx,8:end);

    [ShuffleRcoord,ShuffleCcoord] = ind2sub(size(Shuffleplotdatatemp),find(Shuffleplotdatatemp > -10000));
    
    if length(ShuffleRcoord(:,1))==0
        continue
    end

    for ii = 1:length(ShuffleRcoord)
        Shuffleplotdata2(ii,:) = [ShuffleCcoord(ii) Shuffleplotdatatemp(ShuffleRcoord(ii),ShuffleCcoord(ii))];
    end
    
    Shuffleplotdata{i,1} = Shuffleplotdata2;
    Shuffleplotdata{i,2} = i;
...
Shuffleplotdata(all(cellfun('isempty',Shuffleplotdata),2),:) = [];
...
for i = 1:length(Shuffleplotdata(:,1))

[Shufflep,Shuffletbl,Shufflestats(i,:)] = anovan(Shuffleplotdata{i}(:,2),Shuffleplotdata{i}(:,1),'display','off');


Shufflec = multcompare(Shufflestats(i,:),'display','off');
...
Shufflebin_numbers = cell2mat(Shuffleplotdata(:,2));
Shufflebin_numbers = Shufflebin_numbers.*10;

Shufflebin_numbers = 0:max(Shuffleplotdata{1,1}(:,1))-1;
Shufflebin_numbers = Shufflebin_numbers';
...
[Shufflep,Shuffletbl,Shufflestats(i,:)] = anovan(Shuffleplotdata{i,1}(:,2),Shuffleplotdata{i,1}(:,1),'display','off');
[ShufflecMULT,Shufflem] = multcompare(Shufflestats(i,:),'display','off');
...
Shufflem = [Shufflem Shufflebin_numbers];
[Shufflem_length, ShufflecMULT] = size(Shufflem);
Shufflexdata(:,i) = [Shufflem(:,1)];
Shuffleydata(:,i) = [Shufflem(:,3)];

% % bin_numbers = [];

ADerrors(1:ADm_length,i) = [ADm(:,2)];

Shuffleerrors(1:Shufflem_length,i) = [Shufflem(:,2)];
...
[Shuffler,Shufflec] = size(Shufflexdata);
...
e2 = errorbar(Shuffleydata(:,i),Shufflexdata(:,i),Shuffleerrors(:,i),'.');
...
e2.MarkerFaceColor = "#0072BD";
e2.MarkerEdgeColor = "#0072BD";
e2.Color = "#0072BD";
...
% % writematrix(exportdata, append('/Users/zacharyhoglund/Downloads/AD_','intensity', 'data_plot_coordinates.xlsx'));
% % 
% % writematrix(exportdata2, append('/Users/zacharyhoglund/Downloads/AD_','intensity','_data_plot_coordinates_binned.xlsx'));
% % 
% % 
