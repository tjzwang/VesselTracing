...
outputname = 'C:\Users\zh624\Desktop\AD Blood Vessel Tracing\Human2131\Shuffled_data_imp_fit\Human2131_imp_fit';
...
std_sens = 1;
    ...
    frame_data = surface_data(z:(z+f),10);
    frame_median = median(frame_data);
    idx3(n,:) = [frame_median z z+f];
    bin_edges(n,:) = [frame_median surface_data(idx3(n,2),5) surface_data(idx3(n,3),5)];
      else
    end
    end
    [bin_edges,sort_idz] = sortrows(bin_edges,1);
    [r,c] = size(bin_edges);
    bin_size = round(r/numbins); 
    lower_bound = 0;
    for 1:numbins
        if i ~= 10
        upper_bound = binsize + upper_bound;
