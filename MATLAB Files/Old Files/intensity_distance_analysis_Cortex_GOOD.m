...
%              [  X    Y    Z   MedianNormalizedIntensity    DistanceAlongVessel    DistanceFromSurface   Cortex Distance  ]
...
tau_folder = 'C:\Users\zh624\Desktop\AD Blood Vessel Tracing\Human2131\Tau Coordinates';

% VESSEL DISTANCE TRANSFORMS:

distance_folder = 'C:\Users\zh624\Desktop\AD Blood Vessel Tracing\Human2131\DT Coordinates';

% VESSEL DATA:

vessel_folder = 'C:\Users\zh624\Desktop\AD Blood Vessel Tracing\Human2131\vessel coordinates';
...
folder_dir = '/Users/zacharyhoglund/Desktop/Arivis/Human2131/MATLAB Test Cortex Output';
...
final_name =  'MATLAB_Test_out_data_cortex_vessel' ;
...
waitbar(0,f,'Reading Cortex Data');
fcontent = dir(fullfile(cortex_folder, '*.txt')); %fcontent is a column vector of structures

sourcefolder = cortex_folder;
...
for i = 1:n
   ...
   filename = fcontent(i).name;
   cortex_data_single = [];
   cortex_data_single = readmatrix(fullfile(sourcefolder,filename));
   cortex_data{i,1} = cortex_data_single;
   waitbar(i/n,f,sprintf('Reading Cortex Data: %d %%',floor(i/n*100)));
...
%%
cortex_data_orig = cortex_data;


%%

waitbar(0,f,'Getting Statistics for Vessel 1');
k = length(fcontent');
...
cortex_data = cortex_data_orig;

cortex_data = cell2mat(cortex_data(1,1));
...
C = find(~ismember(cortex_data(:,1:3),new_Tau_data(:,1:3),'rows'));

cortex_data(C,:) = [];

C = find(~ismember(new_Tau_data(:,1:3),cortex_data(:,1:3),'rows'));
...
new_Tau_data = sortrows(new_Tau_data,2);
new_Tau_data = sortrows(new_Tau_data,3);
cortex_data = sortrows(cortex_data,1);
cortex_data = sortrows(cortex_data,2);
cortex_data = sortrows(cortex_data,3);

new_Tau_data(:,7) = cortex_data(:,4);
...
cortex_data(:,1) = ((cortex_data(:,1)./dist_xdim).*(xup - xlow))+xlow;
...
cortex_data(:,2) = ((cortex_data(:,2)./dist_ydim).*(yup - ylow))+ylow;
...
cortex_data(:,3) = ((cortex_data(:,3)./dist_znum).*(zup - zlow))+zlow;
...
Tau_data = [Tau_data(:,1:3) norm_int  Tau_data(:,5:7)];
final_data = Tau_data;
