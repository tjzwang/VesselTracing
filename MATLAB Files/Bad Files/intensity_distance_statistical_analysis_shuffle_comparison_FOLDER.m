...
sourcefolder = 'C:\Users\zh624\Desktop\Zach - Files\Hyman Lab Work Files\BloodVesselTracing\Stat Analysis Data';
...
% away
idx = find( Tau_data1(:,6) <= 0);
Tau_data1(idx,:) = [];
idx = find(Tau_data1(:,6) > ves_dist );
Tau_data1(idx,:) = [];

if num_file >=2
idx = find( Tau_data2(:,6) <= 0);
Tau_data2(idx,:) = [];
idx = find(Tau_data2(:,6) > ves_dist );
Tau_data2(idx,:) = [];
else
end

    if num_file >=3
    idx = find( Tau_data3(:,6) <= 0);
    Tau_data3(idx,:) = [];
    idx = find(Tau_data3(:,6) > ves_dist );
    Tau_data3(idx,:) = [];
    else
    end

        if num_file >=4
        idx = find( Tau_data4(:,6) <= 0);
        Tau_data4(idx,:) = [];
        idx = find(Tau_data4(:,6) > ves_dist );
        Tau_data4(idx,:) = [];
        end


if num_file >=2
Tau_data2 = [Tau_data2(:,1:4) (Tau_data2(:,5)+(max(Tau_data1(:,5)))) Tau_data2(:,6)];
else
Tau_data2 = [];
end

    if num_file >=3
Tau_data3 = [Tau_data3(:,1:4) (Tau_data3(:,5)+(max(Tau_data2(:,5)))) Tau_data3(:,6)];
    else
    Tau_data3 = [];
    end

        if num_file >=4
        Tau_data4 = [Tau_data4(:,1:4) (Tau_data4(:,5)+(max(Tau_data3(:,5)))) Tau_data4(:,6)];
        else
        Tau_data4 = [];
        end

Tau_data = [Tau_data1;Tau_data2;Tau_data3;Tau_data4];
...
% filter data to only include tau within 5 microns of vessel.
surface_idx = find(group_Tau_data(:,6) < 5);
