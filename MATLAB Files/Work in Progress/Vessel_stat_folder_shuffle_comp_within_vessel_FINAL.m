...
sourcefolder = 'C:\Users\zh624\Desktop\AD Blood Vessel Tracing\Human2131\Int_dist_within_vessel';
...
% Folder:
outputfoldername = 'C:\Users\zh624\Desktop\AD Blood Vessel Tracing\Human2131\Stat_data_in_vessel_percent';
% File:
outputname = 'Human2131_in_vessel';
...
nbins = round((ves_dist - min(dist_Tau_data(:,6)))/res);

% get bin edges for grouping the tau data by intensity
[N,edges8,bin2] = histcounts(dist_Tau_data(:,6),nbins);

for i = 1:(length(edges8) - 1)
    if i ~= (length(edges8) - 1)
    idx = find(dist_Tau_data(:,6) >= edges8(:,i)  & dist_Tau_data(:,6) < edges8(:,i+1)) ;
    else
    idx = find( dist_Tau_data(:,6) >= edges8(:,i)  & dist_Tau_data(:,6) <= edges8(:,i+1)) ;
    end
    bin2(idx) = mean([edges8(:,i) edges8(:,i+1)]);
    bin_type(i) = mean([edges8(:,i) edges8(:,i+1)]);
end


group_Tau_data = [dist_Tau_data(:,1:7) bin2 zeros(length(bin),1) zeros(length(bin),1) dist_Tau_data(:,11)];
...
xdata = zeros(numbins,length(dist_groups));
ydata = zeros(numbins,length(dist_groups));
errors = zeros(numbins,length(dist_groups));

row_has_empty = any(cellfun(@isempty, dist_groups), 2);  %find them
dist_groups(row_has_empty,:) = [];   %delete them
...
bins_present = unique(dist_groups{i}(:,8));
bins_present = sortrows(bins_present(:,1)).*10;
...
% % e = errorbar(nonzeros(xdata(:,i)),nonzeros(ydata(:,i)),nonzeros(errors(:,i)),'horizontal','.');
% % hold on
...
e = errorbar(bin_type,xdata(i,:),errors(i,:),'.');
