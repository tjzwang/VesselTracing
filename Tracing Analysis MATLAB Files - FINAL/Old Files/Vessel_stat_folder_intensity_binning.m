...
content = dir(fullfile(Tau_sourcefolder, '*.txt')); %fcontent is a column vector of structures
...
for i = frame_w:frame_w:(ceil(max(Tau_data(:,5)/frame_w))*frame_w)
    ...
    idx = find(Tau_data(:,5) < i & Tau_data(:,5) >= (i - frame_w));
     ...
     if ~isempty(idx)
     for d = 1:length(idx)
    binned_data{g}(d,:) = [vesnum g (i - frame_w) i (Tau_data(idx(d),:))];
     end
        else
        binned_data{g}(d,:) = [vesnum g (i - frame_w) i zeros(1,length(Tau_data(1,:)))];
     end
    g = g+1;
    idx = [];
    d = 1;
...
for i = 1:length(binned_data)
   ...
   new_data(endrow+i,1:4) = [vesnum    i   new_data(endrow+i-1,3)+frame_w      new_data(endrow+i-1,4)+frame_w];
   end
