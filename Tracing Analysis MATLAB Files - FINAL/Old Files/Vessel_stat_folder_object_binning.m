...
waitbar(0.3,f,sprintf('Binning Data for Vessel %d/%d',b,n));
...
for i = frame_w:frame_w:maxlength
    ...
    idx2 = [];
     ...
     for d = 1:length(idx)
     Tau_binned_data{g}(d,:) = [vesnum g (i - frame_w) i (Tau_data(idx(d),:))];
     end

     if ~isempty(idx2(:,g))
     for d = 1:length(idx2)
     Distance_binned_data{g}(d,:) = [vesnum g (i - frame_w) i (Distance_data(idx2(d),:))];
     end
     else
...
for i = 1:length(Tau_binned_data)
   ...
   new_data(endrow+i,1:4) = [b    i   new_data(endrow+i-1,3)+frame_w   new_data(endrow+i-1,4)+frame_w      zeros(1,((ves_dist-min_dist)/res))];
   end
